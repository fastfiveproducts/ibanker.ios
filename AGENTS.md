...
- **Money entry** (#68): every money field goes through `MoneyField` (`ViewSupport/MoneyField.swift`) — never a raw `TextField` — so the iPad docked-pad substitution (iPadOS 26 presents the system `.numberPad` nondeterministically; the pad restores the #42 one-tap flow) and #6's K/M keys apply uniformly. Commit semantics are per-field: continuous by default (required wherever the KeyboardActionBar has an `apply`/`cancel` closure), end-editing only for PlayerView's salary (its `onChange` live-syncs the event log). The file's doc comment carries the full rationale, including the two focus-bridge traps a rework must not re-learn.
