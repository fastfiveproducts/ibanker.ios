...
### Screenshots
App Store screenshots are generated, not hand-captured: `tools/generate-screenshots.sh` re-creates the committed `store-listing/screenshots/` sets end-to-end on the dedicated capture simulators (`ASC_69_iBanker` / `ASC_13_iBanker`, created on demand), seeding a demo game through the app's own persistence (`ScreenshotMode`, DEBUG-only) and validating exact accepted ASC dimensions + no alpha (#55). The script is iBanker's #55 single-class original (a recorded divergence — the launch argument `-ibScreenshotScreen` is hardcoded in it and must stay identical to `ScreenshotMode.launchArgument`); re-adopt the template's two-class tooling at the leg that adds an iPad set (see TEMPLATE.md, and read template.ios#256 first). `tools/validate-screenshots.swift` is the template's copy as-is; validation proves dimensions and alpha, never content — always open the images.
